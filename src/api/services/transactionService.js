...
export const transactionService = {
  ...
  myList: () => api.get('/api/v1/my-transactions').then(r => r.data),
  byId: (id) => api.get(`/api/v1/transaction/${id}`).then(r => r.data),
  all: () => api.get('/api/v1/all-transactions').then(r => r.data),
  cancel: (id) => api.post(`/api/v1/cancel-transaction/${id}`).then(r => r.data),
